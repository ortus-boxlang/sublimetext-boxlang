"""
Inline documentation system for BoxLang.
Shows popup documentation on F1 and hover.
"""
import sublime
...
def build_links(links):
    """Build HTML links from link data."""
...
def build_doc_html(inline_doc, doc_type):
    """Build documentation HTML from template."""
...
def get_on_navigate(view, docs, doc_type, current_index, pt):
    """Get navigation handler for documentation popup."""
...
def display_documentation(view, docs, doc_type, pt=-1, current_index=0):
    ...
    if doc_type == 'completion_doc':
        if doc_window and doc_window == 'completion_doc':
            ...
            view.show_popup(doc_html, flags=sublime.COOPERATE_WITH_AUTO_COMPLETE, max_width=480, on_navigate=on_navigate, on_hide=lambda: on_hide(view, doc_region_id))
    else:
        if doc_regions and utils.get_setting('boxlang_inline_doc_regions_highlight'):
...
class BoxlangInlineDocumentationCommand(sublime_plugin.TextCommand):
    """Show inline documentation popup."""
    ...
    def run(self, edit, doc_type='inline_doc', pt=None):
        ...
        diff = time.time() - tick
        if utils.get_setting('boxlang_log_doc_time'):
            print('BoxLang: inline documentation completed in ' + '{:.0f}'.format(diff * 1000) + 'ms')