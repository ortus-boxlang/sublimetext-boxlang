...
def show_errors(view, file_path, errors):
    ...
    content = f"BoxLang Parse Errors - {file_path}\n"
    content += "=" * 60 + "\n\n"
    ...
    for i, error in enumerate(errors):
        ...
        content += f"Error {i + 1} (line {line}, col {col}):\n"
        content += f"  {message}\n\n"
    ...
    view.window().run_command("show_panel", {"panel": f"output.{_PANEL_NAME}"})
...
def _navigate_to_error(view):
    ...
    sublime.status_message(f"BoxLang error { _current_error_index + 1} of {len(_error_regions)} (line {line})")
