...
def get_goto_boxlang_file(boxlang_view):
    ...
    if boxlang_view.view.match_selector(boxlang_view.position, 'meta.function-call.support.boxlang'):
        func_name, func_region, args_region = boxlang_view.get_function_call(boxlang_view.position, True)
        return boxlang_view.GotoBoxlangFile('https://boxlang.ortusbooks.com/boxlang-language/reference/built-in-functions/{}'.format(func_name), None)
    if boxlang_view.view.match_selector(boxlang_view.position, 'meta.tag.boxlang,meta.tag.script.boxlang'):
        tag_name = utils.get_tag_name(boxlang_view.view, boxlang_view.position)
        if tag_name:
            return boxlang_view.GotoBoxlangFile('https://boxlang.ortusbooks.com/boxlang-language/reference/components/{}'.format(tag_name), None)
    if boxlang_view.view.match_selector(boxlang_view.position, 'string.quoted'):
        word = boxlang_view.view.word(boxlang_view.position)
        dot_path = boxlang_view.view.substr(word)
        if '.' in dot_path:
            file_path = component_index.resolve_path(boxlang_view.project_name, boxlang_view.file_path, dot_path)
    ...
    def run(self, edit):
        ...
        if goto_info:
            if goto_info.file_path.startswith('http'):
                ...
                self.view.window().run_command('show_overlay', {'overlay': 'goto', 'text': goto_info.symbol})