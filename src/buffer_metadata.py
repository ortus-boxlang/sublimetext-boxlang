...
def get_minimal_file_string(view):
    ...
    if len(tag_component_regions) > 0:
        ...
        current_funct = ''
        for r in view.find_by_selector('meta.function.boxlang, meta.function.body.tag.boxlang meta.tag.argument.boxlang'):
            text = view.substr(r)
        ...
        script_selectors = [('comment.block.documentation.boxlang -meta.class', '\n'), ('meta.class.declaration.boxlang', ' {\n'), ('meta.tag.property.boxlang', ';\n')]
...
def get_view_metadata(view):
    ...
    extended_meta.update({'functions': extended_meta.get('functions', {}), 'function_file_map': {}, 'properties': extended_meta.get('properties', {}), 'property_file_map': {}})
    ...
    if project_name and extended_meta.get('extends'):
        ...
        extends_file_path = resolve_path(project_name, file_path, extended_meta['extends'])
    ...
    extended_meta['function_file_map'].update({funct_key: file_path for funct_key in extended_meta.get('functions', {})})
    extended_meta['property_file_map'].update({prop_key: file_path for prop_key in extended_meta.get('properties', {})})
