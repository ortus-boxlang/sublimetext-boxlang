...
def _update_status_bar(view):
    ...
    if version:
        view.set_status(_STATUS_KEY_VERSION, f"BoxLang v{version}")
    else:
        if boxlang_cli.is_installed():
    ...
    if project_name and project_name in _indexing_progress:
        ...
        if indexed < total:
            ...
            view.set_status(_STATUS_KEY_INDEXING, f"Indexing: {indexed}/{total} ({pct}%)")
        else:
            view.erase_status(_STATUS_KEY_INDEXING)
    else:
        view.erase_status(_STATUS_KEY_INDEXING)
    ...
    if file_path and file_path in _error_counts:
        ...
        if count > 0:
            view.set_status(_STATUS_KEY_ERRORS, f"BoxLang: {count} error(s)")
        else:
            view.erase_status(_STATUS_KEY_ERRORS)
    else:
        view.erase_status(_STATUS_KEY_ERRORS)
