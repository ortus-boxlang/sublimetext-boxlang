...
class BoxlangFunctionCallParams:
    ...
    def __init__(self, boxlang_view, position):
        ...
        self.function_name, self.function_region, self.params_region = boxlang_view.get_function_call(
            position
        )

        if (
            "support"
            in boxlang_view.view.scope_name(self.function_region.begin())
            .strip()
            .split(" ")[-1]
        ):
            self.support = True
        ...
        start_scope_list = (
            boxlang_view.view.scope_name(self.params_region.begin())
            .strip()
            .split(" ")[:-1]
        )
        ...
        for pt in range(self.params_region.begin() + 1, self.params_region.end()):
            ...
            if boxlang_view.view.scope_name(pt) == separator_scope:
                current_element = boxlang_view.view.substr(
...
class BoxlangView:
    ...
    def determine_type(self):
        ...
        # Dot completions (member and model function completions)
            ...
            self.prefix_start - 1, base_script_scope + " punctuation.accessor.boxlang"
        ):
            self.type = "dot"
    ...
    def set_tag_info(self, tag_in_script=False):
        ...
        if self.view.match_selector(
            self.prefix_start,
            "meta.tag - punctuation.definition.tag.begin, meta.class.declaration.boxlang",
        ):
            if self.view.match_selector(
                self.prefix_start - 1,
                ...
                self.prefix_start, "entity.other.attribute-name.boxlang"
            ):
