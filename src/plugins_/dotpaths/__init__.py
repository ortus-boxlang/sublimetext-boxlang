...
def make_completion(path_part_dict, key, dot_paths, project_name, constructor):
    ...
    if path_part_dict['is_component'] and constructor:
        ...
        component_completion = component_index.get_completions_by_file_path(project_name, dot_paths[full_key.lower()]['file_path']).get('constructor')
    ...
    if not path_part_dict['is_component']:
        completion += '.'
    return (path_part_dict['path_part'] + '\t' + ('component' if path_part_dict['is_component'] else 'path'), completion)
...
def get_completions_by_component_name(boxlang_view, component_name):
    ...
    comp = component_index.get_completions_by_dot_path(boxlang_view.project_name, component_name.lower())
    if not comp:
        folder_path = get_folder_path(boxlang_view, component_name)
        if folder_path:
            comp = component_index.get_completions_by_dot_path(boxlang_view.project_name, folder_path)
    if comp:
        filtered_completions = []
        for completion in comp.get('functions', []):
            if not completion.get('private', False):
                filtered_completions.append((completion.get('key', '') + '\t' + completion.get('hint', ''), completion.get('content', '')))
...
def get_script_completions(boxlang_view):
    ...
    if not boxlang_view.project_name or boxlang_view.project_name not in projects:
        return None
    if boxlang_view.view.match_selector(boxlang_view.position - 1, 'meta.import.boxlang variable.other.import.boxlang'):
        r = utils.get_scope_region_containing_point(boxlang_view.view, boxlang_view.position - 1, 'meta.import.boxlang')
        if r:
    ...
    if boxlang_view.view.match_selector(boxlang_view.position - 1, 'meta.instance.constructor.boxlang'):
        r = utils.get_scope_region_containing_point(boxlang_view.view, boxlang_view.position - 1, 'meta.instance.constructor.boxlang')
        if r:
    ...
    if boxlang_view.view.match_selector(boxlang_view.position, 'meta.function-call.support.createcomponent.boxlang string.quoted'):
        r = utils.get_scope_region_containing_point(boxlang_view.view, boxlang_view.position, 'string.quoted')
        if r:
...
def get_dot_completions(boxlang_view):
    ...
    component_name = None
    if boxlang_view.dot_context[0].name == 'createobject' and boxlang_view.view.match_selector(boxlang_view.prefix_start - 2, component_selector):
        component_name = get_component_name_from_args(boxlang_view.view.substr(boxlang_view.dot_context[0].args_region))
    elif boxlang_view.view.match_selector(boxlang_view.prefix_start - 2, constructor_selector):
        ...
        component_tuple = find_component_by_var_assignment(boxlang_view, boxlang_view.prefix_start, boxlang_view.dot_context[0].name)
...
def get_component_name_from_args(args_text):
    ...
    import re
    match = re.search('["\\\']component["\\\']\\s*,\\s*["\\\']([^"\\\']+)["\\\']', args_text)
    if match:
...
def find_component_by_var_assignment(boxlang_view, position, variable_name):
    ...
    var_assignment = utils.find_variable_assignment(boxlang_view.view, position, variable_name)
    if var_assignment:
        assign_text = boxlang_view.view.substr(sublime.Region(var_assignment.begin(), var_assignment.end() + 50))
        import re
        match = re.search('=\\s*new\\s+([\\w.]+)', assign_text)
        if match:
