...
def get_tag_attributes(boxlang_view):
    ...
    if (
        boxlang_view.tag_attribute_name is None
        or boxlang_view.tag_location == "tag_attribute_name"
    ):
        completion_list = completions.get("boxlang_tag_attributes", {}).get(
            tag_name, None
        )
        if completion_list:
            return boxlang_view.CompletionList(completion_list, 0, False)
    elif (
        tag_name in completions.get("boxlang_tag_attribute_values", {})
        and boxlang_view.tag_attribute_name
        in completions["boxlang_tag_attribute_values"].get(tag_name, {})
    ):
        completion_list = completions["boxlang_tag_attribute_values"][tag_name][
            boxlang_view.tag_attribute_name
        ]
        return boxlang_view.CompletionList(completion_list, 0, False)
...
def get_script_completions(boxlang_view):
    ...
    if boxlang_view.view.match_selector(
        ...
        "meta.function-call.arguments.boxlang,meta.function-call.arguments.method.boxlang",
    ):
        completion_list.append(
            sublime.CompletionItem(
                ...
                kind=(sublime.KIND_ID_VARIABLE, "v", "BoxLang"),
            )
        )

    completion_list.extend(
        completions.get("boxlang_functions", {}).get(utils.get_setting("boxlang_bif_completions"), [])
    )
...
def get_dot_completions(boxlang_view):
    ...
    completion_list = completions.get("boxlang_member_functions", {}).get(
        utils.get_setting("boxlang_bif_completions"), []
    )
    return boxlang_view.CompletionList(completion_list, 0, False)
...
def load_completions():
    ...
    for tag_name in sorted(tags_data.keys()):
        ...
        completions["boxlang_tags"].append(
            make_tag_completion(tag_name, required_attrs)
        )
        completions["boxlang_tags_in_script"].append(
            make_tag_completion(tag_name, required_attrs)
        )

        completions["boxlang_tag_attributes"][tag_name] = [
            sublime.CompletionItem(
                a,
                "required",
                a + '="$1"',
        ...
        completions["boxlang_tag_attributes"][tag_name].extend(
            [
                sublime.CompletionItem(
                    a,
                    "optional",
        ...
        for attribute_name in sorted(tag_attribute_values.keys()):
            completions["boxlang_tag_attribute_values"][tag_name][attribute_name] = [
                sublime.CompletionItem(
                    v,
                    attribute_name,
                    v,
    ...
    for funct in sorted(functions_data.keys()):
        ...
        completions["boxlang_functions"]["basic"].append(
            sublime.CompletionItem(
                funct,
                "boxlang.fn",
                funct + "($0)",
        ...
        completions["boxlang_functions"]["required"].append(
            sublime.CompletionItem(
                funct,
                "boxlang.fn",
                funct + required_snippet,
                sublime.COMPLETION_FORMAT_SNIPPET,
                kind=(sublime.KIND_ID_FUNCTION, "f", "function"),
                details=description,
            )
        )
        completions["boxlang_functions"]["full"].append(
            sublime.CompletionItem(
                funct,
                "boxlang.fn",
                funct + full_snippet,
    ...
    for member_type in sorted(member_data.keys()):
        for funct in sorted(member_data[member_type].keys()):
            ...
            completions["boxlang_member_functions"]["basic"].append(
                sublime.CompletionItem(
                    funct,
                    member_type + ".fn",
                    funct + "($0)",
            ...
            completions["boxlang_member_functions"]["required"].append(
                sublime.CompletionItem(
                    funct,
                    member_type + ".fn",
                    funct + required_snippet,
            ...
            completions["boxlang_member_functions"]["full"].append(
                sublime.CompletionItem(
                    funct,
                    member_type + ".fn",
                    funct + full_snippet,
...
def make_tag_completion(tag, required_attrs):
    ...
    return sublime.CompletionItem(
        ...
        tag + attrs,
        sublime.COMPLETION_FORMAT_SNIPPET,
        kind=(sublime.KIND_ID_MARKUP, "t", "BoxLang"),
    )
