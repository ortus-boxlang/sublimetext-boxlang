...
def _get_component_member_completions(component_path, boxlang_view):
    ...
    for func_name, func_meta in sorted(functions.items()):
        ...
        if completion_style == 'basic':
            ...
            content = '{}({}$0)'.format(func_name, snippet_args)
        completions.append(sublime.CompletionItem(func_name if completion_names == 'basic' else '{}():{}'.format(func_name, return_type) if return_type else func_name, hint, content, sublime.COMPLETION_FORMAT_SNIPPET, kind=(sublime.KIND_ID_FUNCTION, 'm', component_path.split('.')[-1]), details=func_meta.get('description', '')))
...
def _get_builtin_type_completions(type_name, boxlang_view):
    ...
    completions = []
    type_methods = {'string': [('len\tnumeric', 'len()'), ('length\tnumeric', 'length()'), ('left\tstring', 'left($1)$0'), ('right\tstring', 'right($1)$0'), ('mid\tstring', 'mid($1, $2)$0'), ('trim\tstring', 'trim()'), ('lCase\tstring', 'lCase()'), ('uCase\tstring', 'uCase()'), ('replace\tstring', 'replace($1, $2)$0'), ('find\tnumeric', 'find($1)$0'), ('split\tarray', 'split($1)$0'), ('substring\tstring', 'substring($1, $2)$0'), ('toNumeric\tnumeric', 'toNumeric()'), ('toBoolean\tboolean', 'toBoolean()'), ('toString\tstring', 'toString()')], 'array': [('len\tnumeric', 'len()'), ('length\tnumeric', 'length()'), ('append\tvoid', 'append($1)$0'), ('prepend\tvoid', 'prepend($1)$0'), ('delete\tvoid', 'delete($1)$0'), ('sort\tarray', 'sort()'), ('reverse\tarray', 'reverse()'), ('slice\tarray', 'slice($1, $2)$0'), ('find\tnumeric', 'find($1)$0'), ('contains\tboolean', 'contains($1)$0'), ('isEmpty\tboolean', 'isEmpty()')], 'struct': [('len\tnumeric', 'len()'), ('length\tnumeric', 'length()'), ('keyArray\tarray', 'keyArray()'), ('valueArray\tarray', 'valueArray()'), ('delete\tvoid', 'delete($1)$0'), ('clear\tvoid', 'clear()'), ('isEmpty\tboolean', 'isEmpty()'), ('keyExists\tboolean', 'keyExists($1)$0')], 'query': [('len\tnumeric', 'len()'), ('length\tnumeric', 'length()'), ('columnArray\tarray', 'columnArray()'), ('addColumn\tvoid', 'addColumn($1, $2)$0'), ('deleteRow\tvoid', 'deleteRow($1)$0')], 'numeric': [('abs\tnumeric', 'abs()'), ('ceiling\tnumeric', 'ceiling()'), ('floor\tnumeric', 'floor()'), ('round\tnumeric', 'round()'), ('toString\tstring', 'toString()')]}
    methods = type_methods.get(type_name, [])
    for key, content in methods:
        parts = key.split('\t')
        name = parts[0]
        hint = parts[1] if len(parts) > 1 else ''
        completions.append(sublime.CompletionItem(name, hint, content, sublime.COMPLETION_FORMAT_SNIPPET, kind=(sublime.KIND_ID_FUNCTION, 'f', type_name)))
    return completions
