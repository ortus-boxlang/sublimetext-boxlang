...
# Cache of variable name -> component mappings
variable_mappings = {}
...
def build_variable_mappings(project_name):
    ...
    for folder_config in cfc_folders:
        ...
        # Walk folder and build mappings
        for root, dirs, files in os.walk(folder_path):
            for f in files:
                if f.endswith(".bx"):
                    cfc_name = f[:-3]  # Remove .bx extension
                    ...
                    for template in variable_templates:
                        ...
                        if dot_path:
                            ...
                            if metadata:
                                variable_mappings[project_name][var_name.lower()] = {
                                    "dot_path": dot_path,
                                    "metadata": metadata,
                                    "accessors": accessors
                                }
...
def get_script_completions(boxlang_view):
    ...
    # Check if we're typing a variable name followed by a dot
    if boxlang_view.view.match_selector(
        boxlang_view.position - 1, "variable.other.readwrite.boxlang"
    ):
        ...
        if word_text in variable_mappings[boxlang_view.project_name]:
            ...
            completions = _get_component_completions(
                boxlang_view.project_name,
                mapping["dot_path"],
                mapping["metadata"],
                mapping["accessors"]
            )
...
def get_dot_completions(boxlang_view):
    ...
    # Check if first element in dot context is a known variable
...
def get_inline_documentation(boxlang_view, doc_type):
    ...
    # Check if cursor is on a variable name
    ...
    for func_name, func_meta in functions.items():
        ...
        completions.append(
            sublime.CompletionItem(
                ...
                kind=(sublime.KIND_ID_FUNCTION, "m", dot_path.split(".")[-1]),
                details=func_meta.get("description", "")
            )
        )

    # Add property accessors if enabled
...
def _build_documentation(boxlang_view, var_name, mapping, region):
    ...
    doc = {
        ...
        "html": {
            "header": f"Component: <span class=\"entity_name_class\">{dot_path.split('.')[-1]}</span>",
            "body": f"<p>Dot path: <code>{dot_path}</code></p>",
            "links": []
        }
    }
    ...
    if metadata.get("extends"):
        doc["html"]["body"] += f"<p>Extends: <code>{metadata['extends']}</code></p>"
    ...
    if functions:
        ...
        for func_name, func_meta in sorted(functions.items()):
            ...
            args = func_meta.get("args", [])
            return_type = func_meta.get("return_type", "")
            arg_str = ", ".join([f"{a.get('name', '')}" + (f": {a.get('type', '')}" if a.get('type') else "") for a in args])
            sig = f"{func_name}({arg_str})"
            if return_type:
                sig += f": {return_type}"
            doc["html"]["body"] += f"<p><code>{sig}</code></p>"

    # Add link to docs
    doc["html"]["links"] = [
        {
            "href": f"https://boxlang.ortusbooks.com/boxlang-language/reference/components/{dot_path.split('.')[-1]}",
            "text": "Open in Docs"
        }
    ]
