...
KNOWN_TYPES = {'string', 'numeric', 'number', 'boolean', 'array', 'struct', 'map', 'query', 'date', 'datetime', 'void', 'any', 'xml', 'binary', 'function', 'closure', 'lambda', 'java', 'uicomponent'}
BIF_RETURN_TYPES = {'arrayNew': 'array', 'arrayLen': 'numeric', 'structNew': 'struct', 'mapNew': 'map', 'queryNew': 'query', 'now': 'datetime', 'createDate': 'date', 'createDateTime': 'datetime', 'isDefined': 'boolean', 'isNull': 'boolean', 'isStruct': 'boolean', 'isArray': 'boolean', 'isQuery': 'boolean', 'isDate': 'boolean', 'isNumeric': 'boolean', 'isBoolean': 'boolean', 'isClosure': 'boolean', 'isCustomFunction': 'boolean', 'toString': 'string', 'toNumeric': 'numeric', 'toBoolean': 'boolean', 'toScript': 'string', 'getMetaData': 'struct', 'getFunctionCalledName': 'string', 'getBaseTemplatePath': 'string', 'getCurrentTemplatePath': 'string', 'getDirectoryFromPath': 'string', 'getFileFromPath': 'string', 'trim': 'string', 'len': 'numeric', 'left': 'string', 'right': 'string', 'mid': 'string', 'lCase': 'string', 'uCase': 'string', 'dateFormat': 'string', 'timeFormat': 'string', 'encodeForHTML': 'string', 'decodeFromHTML': 'string', 'hash': 'string', 'rand': 'numeric', 'randRange': 'numeric', 'abs': 'numeric', 'ceiling': 'numeric', 'floor': 'numeric', 'round': 'numeric', 'max': 'numeric', 'min': 'numeric', 'pi': 'numeric', 'inputBaseN': 'numeric', 'formatBaseN': 'string'}
...
class TypeResolver:
    ...
    def _check_new_expression(self, expr):
        ...
        match = re.match('new\\s+([\\w.]+)', expr)
        if match:
            component_path = match.group(1)
            return 'component:{}'.format(component_path)
        return None
    ...
    def _check_create_object(self, expr):
        ...
        match = re.match('createObject\\s*\\(\\s*["\\\']component["\\\']\\s*,\\s*["\\\']([^"\\\']+)["\\\']', expr)
        if match:
            component_path = match.group(1)
            return 'component:{}'.format(component_path)
        return None
