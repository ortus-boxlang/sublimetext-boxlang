...
class BoxLangEventListener(sublime_plugin.EventListener):
    ...
    def on_post_save_async(self, view):
        if not view.file_name():
            print(
                "BoxLang: file was saved and closed - it is not possible to determine the file path."
            )
            return
    ...
    def on_post_text_command(self, view, command_name, args):
        if command_name == "commit_completion":
            ...
            if view.match_selector(
                pos,
                "meta.tag.boxlang -source.boxlang.script, meta.tag.script.boxlang, meta.tag.script.cf.boxlang, meta.class.declaration.boxlang -meta.class.inheritance.boxlang",
            ):
            ...
            if view.substr(pos - 1) == ":" and view.match_selector(
                pos - 1, "meta.tag.custom.boxlang -source.boxlang.script"
            ):
                view.run_command("auto_complete", {"api_completions_only": True})

            if view.substr(pos - 1) == "." and view.match_selector(
                pos - 1,
                "meta.function-call.support.createcomponent.boxlang string.quoted, entity.other.inherited-class.boxlang, meta.instance.constructor.boxlang",
            ):
                view.run_command("auto_complete", {"api_completions_only": True})
    ...
    def on_query_completions(self, view, prefix, locations):
        if not view.match_selector(locations[0], "embedding.boxlang"):
            return None
    ...
    def on_hover(self, view, point, hover_zone):
        ...
        if not view.match_selector(point, "embedding.boxlang"):
            return

        view.run_command(
            "boxlang_inline_documentation", {"pt": point, "doc_type": "hover_doc"}
        )
...
class CustomHtmlTagCompletions(html_completions.HtmlTagCompletions):
    ...
    def on_query_completions(self, view, prefix, locations):
        ...
        # Only trigger within boxlang tag component functions
        selector = "meta.class.body.tag.boxlang meta.function.body.tag.boxlang -source.boxlang.script -source.sql"
        ...
        if view.match_selector(locations[0], "meta.function.body.tag.boxlang meta.tag - meta.string - punctuation.definition.tag.begin"):
            if ch in ' \f\n\t':
                return self.attribute_completions(view, locations[0], prefix)
            return None

        if view.match_selector(locations[0], "meta.function.body.tag.boxlang - meta.tag, meta.function.body.tag.boxlang punctuation.definition.tag.begin"):
