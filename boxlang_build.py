...
def _get_boxlang_path():
    ...
    candidates = [
        ...
        r"c:\boxlang\bin\boxlang.bat",
        os.path.expandvars(r"${USERPROFILE}\.local\bin\boxlang.bat"),
    ]
        ...
        if with_args:
            def on_done(args):
                self.window.run_command("exec", {
                    "shell_cmd": '{} "{}" {}'.format(bx_path, file_path, args),
                    "file_regex": r"(?:Error compiling.*Line: ([0-9]+) Col: ([0-9]+) - (.*))",
                })
            self.window.show_input_panel("Arguments:", "", on_done, None, None)
        else:
            self.window.run_command("exec", {
                "shell_cmd": '{} "{}"'.format(bx_path, file_path),
                "file_regex": r"(?:Error compiling.*Line: ([0-9]+) Col: ([0-9]+) - (.*))",
            })
...
class BoxlangCompileCommand(sublime_plugin.WindowCommand):
    ...
    def run(self, scope="file"):
        ...
        self.window.run_command("exec", {
            "shell_cmd": '{} compile --source "{}" --target "{}"'.format(bx_path, file_path, target),
            "file_regex": r"(?:Error compiling.*Line: ([0-9]+) Col: ([0-9]+) - (.*))",
        })
...
class BoxlangDebugCommand(sublime_plugin.WindowCommand):
    ...
    def run(self):
        ...
        self.window.run_command("exec", {
            "shell_cmd": '{} --bx-debug "{}"'.format(bx_path, file_path),
            "file_regex": r"(?:Error compiling.*Line: ([0-9]+) Col: ([0-9]+) - (.*))",
        })
...
class BoxlangAuditCommand(sublime_plugin.WindowCommand):
    ...
    def run(self):
        ...
        self.window.run_command("exec", {
            "shell_cmd": '{} featureaudit --source "{}"'.format(bx_path, file_path),
        })
